Bolagato Core version 0.13.2 is now available from:

  <https://download.bolagato.org/bolagato-0.13.2.1/>
...
updates will be released anymore. Without security updates, using a bolagato
...
ECDSA signatures inside Bolagato transactions now use validation using
...
Previous versions of Bolagato Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Bolagato Core was to raise the default minimum
relay fee.

Bolagato Core 0.13.2 will have a strict maximum size on the mempool. The
...
Bolagato Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running bolagatod without having to do any manual configuration.
...
Bolagato Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Bolagato Core will relay transactions with
...
entirely in Bolagato Core 0.13, and it is at this time undecided whether the
...
Bolagato Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Bolagato Core automatically creates a hidden service to listen on, without
manual configuration. Bolagato Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Bolagato Core is listening, and
...
Bolagatod can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Bolagato
Core to use floating fees. By default, Bolagato Core will use floating
fees.
...
(default: `0.0002` BQT/kB).

At all times, Bolagato Core will cap fees at `-maxtxfee=<x>` (default:
0.10) BQT.
Furthermore, Bolagato Core will never create transactions paying less than
...
`bolagato.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /bolagatorpc>
        ...
        # optional bypass bolagatod rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< bolagatorpc:<password>
    </Location>
...
Restarting bolagatod will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in bolagato.conf
...
bolagato-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/bolagato-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Bolagato Core code base has
...
- `bolagato-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `bolagato-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Bolagato scripting system that in combination with
...
Bolagato transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Bolagato Core.
...
The two phases are distinct in the Bolagato-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Bolagato wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Bolagato.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Bolagato block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of bolagatos they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of bolagatos being spent is exactly
...
- **Increased security for multisig:** Bolagato addresses (both P2PKH addresses
  ...
  believed to be provided by Bolagato's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Bolagato's current ledger (called the UTXO set),
  ...
  note that Bolagato Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Bolagato users to individually opt-in to almost any change in the Bolagato
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Bolagato Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Bolagato
client to Bolagato Core 0.13.2 to signal for segwit support. When a
...
Since Bolagato Core 0.10.0, nodes have defaulted to only relaying and
...
- `bolagato-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `bolagato-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
